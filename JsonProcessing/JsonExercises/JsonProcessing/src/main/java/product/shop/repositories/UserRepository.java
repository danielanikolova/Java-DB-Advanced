...
//    List<Object[]>CategoriesByProductsCount();
    ...
    @Query(value = "select u.id, count(p.id) as product_count from users as u\n" +
            "inner join products as p on u.id=p.seller_id\n" +
            "where p.buyer_id is not null\n" +
            "group by u.id\n" +
            "order by product_count desc, u.last_name asc", nativeQuery = true)
    List<Object[]>FindWithAtLeastOneSoldProduct();
